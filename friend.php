...
class Friend{
    ...
    public function is_already_friends($my_id, $user_id){
        try{
            $sql = "SELECT * FROM `friends` WHERE (my_friend_id = :my_id AND my_id = :frnd_id) OR (my_friend-id = :frnd_id AND my_id = :my_id)";
    ...
    public function make_friends($my_id, $user_id){
        ...
        try{
            ...
            if($delete_stmt->execute()){
                ...
                $sql = "INSERT INTO `friends`(user_one, my_id) VALUES(?, ?)";
                $stmt = $this->db->prepare($sql);
                $stmt->execute([$my_id, $user_id]);
                header('Location: user_profile.php?id='.$user_id);
                exit;
    ...
    public function delete_friends($my_id, $user_id){
        try{
            $delete_friends = "DELETE FROM `friends` WHERE (my_friend_id = :my_id AND my_id = :frnd_id) OR (user_one = :frnd_id AND my_id = :my_id)";
    ...
    public function get_all_friends($my_id, $send_data){
        try{
            $sql = "SELECT * FROM `friends` WHERE my_friend_id = :my_id OR my_id = :my_id";
            $stmt = $this->db->prepare($sql);
            $stmt->bindValue(':my_id',$my_id, PDO::PARAM_INT);
            $stmt->execute();
                ...
                if($send_data){
                    ...
                    foreach($all_users as $row){
                        if($row->user_one == $my_id){
                            ...
                            $get_user_stmt->execute([$row->my_id]);
